...
export interface GatewayMediaController {
  ...
  createWebrtcFromOffer(
    sessionId: string,
    params: { sdp: string; direction?: string },
  ): Promise<{ endpointId: string; sdpAnswer: string }>;
  acceptWebrtcAnswer(
    endpointId: string,
  ...
  restartIce(
    endpointId: string,
  ): Promise<{ sdpOffer: string; offerGeneration: number; iceConfiguration?: RtcIceConfiguration }>;
  createRtpOffer(
    sessionId: string,
  ...
  playAndGather(
    sessionId: string,
    params: PlayAndGatherParams,
  ): Promise<GatherResult & { playbackEndpointId?: string }>;
