...
interface MediaEventPublisher {
  sendEvent(
    connectionId: string,
    event: { event: string; sessionId?: string; data?: unknown },
  ): boolean;
}
...
export class MediaSessionService implements GatewayMediaController {
  ...
  private pendingIceRestarts = new Map<
    string,
    Promise<{ sdpOffer: string; offerGeneration: number }>
  >();
  ...
  private async createIceConfiguration(
    client: RtpbridgeClient,
    sessionId: string,
  ): Promise<RtcIceConfiguration | undefined> {
    ...
    if (!mediaIp) {
      throw new MediaUnavailableError(
        "rtpbridge server.info returned no usable IPv4 media_ip for cohosted coturn",
      );
    }
  ...
  async bridge(sessionId: string, params: BridgeParams): Promise<BridgeResult> {
    ...
    if (session.backendId !== target.backendId) {
      throw new MediaActionConflictError(
        "Cannot bridge media sessions on different rtpbridge backends",
      );
    }
    ...
    if (currentSession !== session || currentTarget !== target) {
      ...
      throw new MediaSessionNotFoundError(
        "Media session was destroyed while bridge was being created",
      );
    }
  ...
  async playAndWait(sessionId: string, params: PlayAndWaitParams) {
    const session = this.requireSession(sessionId);
    const played = await this.playToCompletion(
      session,
      params.source,
      params.playbackTimeoutMs ?? 120_000,
    );
    return { played };
  }
  ...
  async leaveMessage(sessionId: string, params: LeaveMessageParams): Promise<LeaveMessageResult> {
    ...
    const release = this.claimEndpointAction(params.endpointId, "leave-message", () =>
      abort.abort(),
    );
  ...
  async injectDtmf(endpointId: string, digit: string) {
    if (!/^[0-9A-D#*]$/i.test(digit))
      throw new MediaRecordingProxyError(400, "digit must be one DTMF character");
    const session = this.requireSessionForEndpoint(endpointId);
  ...
  async listRecordings(
    params: { backendId?: string; startsWith?: string; skip?: number; limit?: number } = {},
  ) {
    const skip = Math.max(0, params.skip ?? 0);
    const limit = Math.min(Math.max(1, params.limit ?? 100), 1000);
    const backends = params.backendId
      ? [await this.mediaServers.resolveBackend(params.backendId)]
      : await this.mediaServers.resolveBackends();

    const byBackend = await Promise.all(
      backends.map(async (backend) => this.listBackendRecordings(backend, params.startsWith)),
    );
    const all = byBackend
      .flat()
  ...
  async mergeRecordings(targets: RecordingMergeTarget[]) {
    ...
    if (targets.length > MAX_RECORDING_MERGE_TARGETS) {
      throw new MediaRecordingProxyError(
        400,
        `recording merge supports at most ${MAX_RECORDING_MERGE_TARGETS} targets`,
      );
    }
  ...
  async deleteRecording(backendId: string, recordingPath: string) {
    ...
    try {
      ...
      throw new MediaRecordingProxyError(
        response.status,
        response.data?.error ?? `rtpbridge recording delete failed with ${response.status}`,
      );
    } catch (err) {
      if (err instanceof MediaRecordingProxyError) throw err;
  ...
  private requireEndpointInSession(sessionId: string, endpointId: string) {
    ...
    if (endpointSessionId !== sessionId || !session.endpoints.has(endpointId)) {
      throw new MediaEndpointNotFoundError(
        `Media endpoint ${endpointId} not found in session ${sessionId}`,
      );
    }
    return session;
  }
  ...
  private async waitForLeaveMessageTerminator(
    ...
    return new Promise((resolve) => {
      ...
      const onDtmf = (event: { endpointId?: string; digit?: string }) => {
        if (
          event.endpointId === params.endpointId &&
          event.digit &&
          terminator.includes(event.digit.toUpperCase())
        ) {
          done("dtmf");
        }
      };
  ...
  private async playToCompletion(
    session: MediaSessionRecord,
    source: string,
    timeoutMs: number,
    signal?: AbortSignal,
  ...
  private async playTone(session: MediaSessionRecord, tone: string, durationMs: number) {
    const playback = await session.client.createToneEndpoint(session.sessionId, {
      tone,
      durationMs,
    });
  ...
  private claimEndpointAction(
    ...
    if (active)
      throw new MediaActionConflictError(
        `Endpoint ${endpointId} already has active ${active.name}`,
      );
    ...
    return () => {
      if (released) return;
      released = true;
      if (this.activeEndpointActions.get(endpointId) === record)
        this.activeEndpointActions.delete(endpointId);
    };
  }
  ...
  private recordingMetadata(session: MediaSessionRecord, filePath: string) {
    ...
    return {
      ...
      filePath,
      recordingPath,
      downloadPath: session.backendId
        ? `/recordings/${encodeURIComponent(session.backendId)}/${encodeRecordingPath(recordingPath)}`
        : "",
    };
  }
  ...
  private async listBackendRecordings(
    backend: RtpbridgeBackend,
    startsWith?: string,
    ...
    do {
      ...
      try {
        const response = await axios.get<{ recordings?: string[]; total?: number }>(
          url.toString(),
          {
            timeout: this.recordingHttpTimeoutMs,
          },
  ...
  private recordingUrl(backend: RtpbridgeBackend, recordingPath: string) {
    return new URL(
      `/recordings/${encodeRecordingPath(recordingPath)}`,
      ensureTrailingSlash(backend.httpUrl),
    ).toString();
  }
  ...
  private async downloadRecordingToFile(
    ...
    if (response.status < 200 || response.status >= 300) {
      ...
      throw new MediaRecordingProxyError(
        response.status,
        `rtpbridge recording download failed with ${response.status}`,
      );
    }
...
async function readPcapHeader(filePath: string) {
  ...
  try {
    ...
    if (bytesRead !== header.length)
      throw new MediaRecordingProxyError(422, "Invalid PCAP segment");
    return header;
  } finally {
    await file.close();
  }
...
function positiveIntegerOrDefault(value: number | undefined, fallback: number | undefined) {
  ...
  if (!Number.isInteger(value) || value <= 0)
    throw new MediaRecordingProxyError(
      400,
      "media action timing and count values must be positive integers",
    );
  return value;
}
...
function recordingProxyError(err: unknown, fallbackMessage: string) {
  if (err instanceof AxiosError) {
    if (err.response) {
      return new MediaRecordingProxyError(
        err.response.status,
