...
export class SessionCommandHandler {
    ...
    switch (method) {
      ...
      case "sip.reject":
        throw new CommandNotImplementedError(
          `${method} is not implemented in this gateway slice yet`,
        );
      case "vad.start":
      case "vad.stop":
      case "amd.start":
  ...
  private async createMediaSession(params: unknown, context: CommandContext) {
    ...
    return this.requireMedia().createSession({
      callId,
      ownerConnectionId: context.controlConnectionId,
    });
  }
  ...
  private async createOutbound(params: unknown, context: CommandContext) {
    ...
    if (!context.controlConnectionId && !receiverUrl) {
      throw new CommandValidationError(
        "receiverUrl is required when sip.createOutbound is not sent over the control WebSocket",
      );
    }
  ...
  private async cancelOutbound(params: unknown, context: CommandContext) {
    if (!this.gateway.cancelOutbound)
      throw new CommandNotImplementedError("sip.cancelOutbound is not implemented");
    const body = requireObject(params);
    return this.gateway.cancelOutbound(
      requiredString(body.outboundAttemptId, "outboundAttemptId"),
      context.controlConnectionId,
    );
  }
  ...
  private async acceptWebrtcAnswer(params: unknown) {
    ...
    if (
      offerGeneration !== undefined &&
      (!Number.isInteger(offerGeneration) || (offerGeneration as number) < 0)
    ) {
  ...
  private async createRtpOffer(params: unknown) {
    ...
    if (
      codecs !== undefined &&
      (!Array.isArray(codecs) || codecs.some((codec) => typeof codec !== "string" || !codec.trim()))
    ) {
  ...
  private async leaveMessage(params: unknown) {
    ...
    return this.requireMedia().leaveMessage(requiredString(body.sessionId, "sessionId"), {
      endpointId: requiredString(body.endpointId, "endpointId"),
      messageSource:
        optionalString(body.messageSource, "messageSource") ??
        optionalString(body.promptSource, "promptSource"),
...
function requiredString(value: unknown, name: string) {
  if (typeof value !== "string" || !value.trim())
    throw new CommandValidationError(`${name} is required`);
  return value;
}
...
function optionalNumber(value: unknown, name: string) {
  ...
  if (typeof value !== "number" || !Number.isFinite(value))
    throw new CommandValidationError(`${name} must be a number`);
  return value;
}
...
function optionalNonNegativeInteger(value: unknown, name: string) {
  ...
  if (parsed !== undefined && parsed < 0)
    throw new CommandValidationError(`${name} must be a non-negative integer`);
  return parsed;
}
...
function optionalPositiveInteger(value: unknown, name: string) {
  const parsed = optionalInteger(value, name);
  if (parsed !== undefined && parsed <= 0)
    throw new CommandValidationError(`${name} must be a positive integer`);
  return parsed;
}
...
function optionalNullableInteger(value: unknown, name: string) {
  ...
  if (!Number.isInteger(value))
    throw new CommandValidationError(`${name} must be an integer or null`);
  return value as number;
}
...
function requiredDirection(value: unknown) {
  ...
  if (!["sendrecv", "recvonly", "sendonly", "inactive"].includes(direction)) {
    throw new CommandValidationError(
      "direction must be one of sendrecv, recvonly, sendonly, inactive",
    );
  }
