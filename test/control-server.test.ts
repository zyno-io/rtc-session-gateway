...
test("control websocket registers routes and supports gateway-initiated requests", async () => {
  ...
  try {
    ...
    await waitFor(() =>
      messages.some((message) => message.type === "response" && message.id === "register-1"),
    );

    const route = hub.findRoute({
...
test("gateway-initiated response must come from the requested control connection", async () => {
  ...
  try {
    ...
    const request = await waitForMessage(
      messagesA,
      (message) => message.type === "request" && message.method === "sip.reinvite",
    );
...
test("pending gateway-initiated request is rejected when its control connection closes", async () => {
  ...
  try {
    ...
    await waitForMessage(
      messages,
      (message) => message.type === "request" && message.method === "sip.invite",
    );
    ws.close();
...
test("websocket-created media sessions receive the owner control connection id", async () => {
  ...
  try {
    ...
    const connected = await waitForMessage(
      messages,
      (message) => message.event === "control.connected",
    );
    ws.send(
      JSON.stringify({
        type: "request",
    ...
    assert.deepEqual(media.createSessionCalls, [
      { callId: "call-1", ownerConnectionId: connected.data.connectionId },
    ]);
  } finally {
    ws.close();
    server.close();
...
test("Drachtio application routing configuration is validated", () => {
  assert.throws(
    () => loadConfig({ DRACHTIO_APP_TAG: "invalid tag" } as NodeJS.ProcessEnv),
    /DRACHTIO_APP_TAG/,
  );
  assert.throws(
    () => loadConfig({ DRACHTIO_ROUTE_FALLBACK_URL: "not a url" } as NodeJS.ProcessEnv),
    /DRACHTIO_ROUTE_FALLBACK_URL/,
  );
  ...
  assert.equal(
    config.DRACHTIO_ROUTE_FALLBACK_URL,
    "http://zynotalk-pilot-server:3000/drachtio/route",
  );
});
...
test("coturn credential configuration defaults to an all-day lifetime and validates overrides", () => {
  ...
  const config = loadConfig({
    COTURN_AUTH_SECRET: "turn-secret",
    COTURN_CREDENTIAL_TTL_SECONDS: "43200",
  } as NodeJS.ProcessEnv);
  assert.equal(config.COTURN_AUTH_SECRET, "turn-secret");
  assert.equal(config.COTURN_CREDENTIAL_TTL_SECONDS, 43_200);
  assert.throws(
    () => loadConfig({ COTURN_CREDENTIAL_TTL_SECONDS: "0" } as NodeJS.ProcessEnv),
    /positive integer/,
  );
});
...
class FakeMediaController implements GatewayMediaController {
  ...
  async createWebrtcFromOffer() {
    return { endpointId: "webrtc-1", sdpAnswer: "answer-sdp" };
  }
  async acceptWebrtcAnswer() {
    return { ok: true as const };
  }
  async acceptWebrtcOffer() {
    return { sdpAnswer: "answer-sdp" };
  }
  async restartIce() {
    return { sdpOffer: "offer-sdp", offerGeneration: 1 };
  }
  async createRtpOffer() {
    return { endpointId: "rtp-1", sdpOffer: "offer-sdp" };
  }
  async createRtpFromOffer() {
    return { endpointId: "rtp-1", sdpAnswer: "answer-sdp" };
  }
  async acceptRtpAnswer() {
    return { ok: true as const };
  }
  async rtpReinvite() {
    return { sdpAnswer: "answer-sdp" };
  ...
  async gather() {
    return { digits: "", reason: "timeout" as const };
  }
  async playAndGather() {
    return { digits: "", reason: "timeout" as const, playbackEndpointId: "file-1" };
  }
  async leaveMessage() {
    return {
  ...
  async stopRecording() {
    return { filePath: "/tmp/rec.pcap", durationMs: 0, packets: 0 };
  }
  async listRecordings() {
    return { recordings: [], total: 0, skip: 0, limit: 100 };
  }
  async downloadRecording() {
    return { status: 200, headers: {}, stream: Readable.from([]) };
  }
