...
test('session commands create outbound SIP calls through the gateway', async () => {
    ...
    assert.deepEqual(result, { sessionId: 'sip-session-1', sipCallId: 'sip-call-1', sdp: 'remote-sdp' });
...
test('session commands route WebRTC and recording operations to media controller', async () => {
    ...
    assert.deepEqual(await commands.execute('webrtc.createOffer', { sessionId: 'media-session-1', direction: 'sendrecv' }), {
        endpointId: 'webrtc-1',
        sdpOffer: 'offer-sdp'
    });
    assert.deepEqual(await commands.execute('webrtc.acceptAnswer', { endpointId: 'webrtc-1', sdp: 'answer-sdp', offerGeneration: 2 }), {
        ok: true
    });
    ...
    assert.deepEqual(await commands.execute('recording.delete', { backendId: 'rtpbridge-0', path: 'call_42.pcap' }), { deleted: true });
    assert.deepEqual(await commands.execute('media.gather', { sessionId: 'media-session-1', endpointId: 'rtp-1', numDigits: 1 }), {
        digits: '7',
        reason: 'digits'
    });
    ...
    assert.deepEqual(await commands.execute('media.bridge', { sessionId: 'media-session-1', targetSessionId: 'media-session-2', direction: 'sendrecv' }), {
        sessionId: 'media-session-1',
        endpointId: 'bridge-1',
        targetSessionId: 'media-session-2',
        targetEndpointId: 'bridge-2'
    });
    assert.deepEqual(await commands.execute('media.unbridge', { endpointId: 'bridge-1' }), { ok: true });
    assert.deepEqual(await commands.execute('dtmf.inject', { endpointId: 'rtp-1', digit: '5' }), { ok: true });
    assert.deepEqual(await commands.execute('endpoint.updateDirection', { endpointId: 'webrtc-1', direction: 'inactive' }), { ok: true });
...
test('session commands validate recording pagination and header errors as bad requests', async () => {
    const commands = new SessionCommandHandler(new CallRegistry(), fakeGateway, new FakeMediaController());
...
test('HTTP media endpoints use the same command surface', async () => {
    ...
    try {
        ...
        const offer = await postJson(server, '/sessions/media-session-1/webrtc/offers', { direction: 'sendrecv' });
        ...
        const gathered = await postJson(server, '/sessions/media-session-1/media/gather', { endpointId: 'rtp-1', numDigits: 1 });
        assert.equal(gathered.status, 200);
        assert.deepEqual(gathered.body, { digits: '7', reason: 'digits' });

        const playAndGathered = await postJson(server, '/sessions/media-session-1/media/play-and-gather', {
            endpointId: 'rtp-1',
            source: 'https://audio.example.com/menu.wav',
            numDigits: 1
        });
        assert.equal(playAndGathered.status, 200);
        assert.deepEqual(playAndGathered.body, { digits: '1', reason: 'digits', playbackEndpointId: 'file-1' });
        ...
        const direction = await postJson(server, '/media/endpoints/webrtc-1/direction', { direction: 'inactive' });
...
test('HTTP recording proxy lists and streams backend recordings', async () => {
    ...
    try {
        ...
        assert.deepEqual(media.calls.at(-1), ['mergeRecordings', [
            { backendId: 'rtpbridge-0', path: 'call_42__seg_0001.pcap' },
            { backendId: 'rtpbridge-1', path: 'call_42__seg_0002.pcap' }
        ]]);
    } finally {
...
test('HTTP outbound SIP endpoint uses the same command surface', async () => {
    ...
    try {
        ...
        assert.deepEqual(response.body, { sessionId: 'sip-session-1', sipCallId: 'sip-call-1', sdp: 'remote-sdp' });
        assert.deepEqual(calls, [
            {
                requestUri: 'sip:15551234567@carrier.example.com',
                sdp: 'local-sdp',
...
const fakeGateway: GatewayController = {
    isConnected: true,
    createOutbound: async () => ({ sessionId: 'sip-session-1', sipCallId: 'sip-call-1', sdp: 'remote-sdp' }),
    reinvite: async () => ({ sdp: 'remote-sdp' }),
    bye: async () => { }
};
...
class FakeMediaController implements GatewayMediaController {
    ...
    acceptWebrtcAnswer = async (endpointId: string, params: { sdp: string; offerGeneration?: number }) => {
    ...
    bridge = async (sessionId: string, params: { targetSessionId: string; direction?: 'sendrecv' | 'recvonly' | 'sendonly' | 'inactive' }) => {
    ...
    gather = async (sessionId: string, params: { endpointId: string; numDigits?: number; timeoutMs?: number; interDigitTimeoutMs?: number; terminator?: string }) => {
    ...
    startRecording = async (sessionId: string, params: { endpointId?: string; filePath: string; recordOutbound?: boolean }) => {
    ...
    listRecordings = async (params?: { backendId?: string; startsWith?: string; skip?: number; limit?: number }) => {
...
async function postJson(server: http.Server, path: string, body: unknown, headers?: Record<string, string>) {
    const port = (server.address() as any).port;
